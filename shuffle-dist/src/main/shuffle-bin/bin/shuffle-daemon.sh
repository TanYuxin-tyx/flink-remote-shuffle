...
################################################################################
# Start/stop a rss daemon.
USAGE="Usage: shuffle-daemon.sh (start|stop|stop-all) (shufflemanager|shuffleworker) [args]"
...
SHUFFLE_PID_DIR="/tmp"
pid=$SHUFFLE_PID_DIR/$DAEMON.pid
...
if [ -z "${LOG_PREFIX}" ]; then
    LOG_PREFIX="${SHUFFLE_LOG_DIR}/${DAEMON}-${id}-${HOSTNAME}"
fi
...
function guaranteed_kill {
  ...
  if command -v timeout &> /dev/null ; then
    # wait 10 seconds for process to stop. By default, we kills the JVM 5 seconds after sigterm.
