...
/** {@link DataPartitionReader} for {@link LocalFileMapPartition}. */
public class LocalFileMapPartitionReader extends BaseDataPartitionReader {

    private static final Logger LOG = LoggerFactory.getLogger(LocalFileMapPartitionReader.class);

    /** File reader responsible for reading data from partition file. */
    private final LocalMapPartitionFileReader fileReader;
    ...
    /**
     * Reads data through the corresponding {@link LocalMapPartitionFileReader} and returns true if
     * there is remaining data to be read with this data partition reader.
     */
    @Override
    ...
    public int compareTo(DataPartitionReader that) {
        int thisQueuedBuffers = getNumberOfQueuedBuffers();
        int thatQueuedBuffers = that.getNumberOfQueuedBuffers();
